...
│   └── src/
│       ├── index.ts                   # Реэкспорты
│       ├── types.ts                   # Публичные интерфейсы: Trajectory, CardMoveOptions, BuilderConfig
│       ├── animations/
...
- ✅ `builders/` → `animations/`, `trajectory/` — внутри core
...
AnimationBuilder.buildMoveAnimation(cards)  // TrajectoryCalculator.calculate() — Last+Invert
...
useCardAnimation() → AnimationBuilder (внутри) → snapshot() / animateMove()
                  ↗                             ↘
             ref(isAnimating)              builder.buildMoveAnimation().play()
```
...
- ❌ **Состояние в AnimationRunner:** `AnimationRunner` не должен хранить глобальное состояние. Создавай новый экземпляр через `builder.buildMoveAnimation()` для каждой анимации.
